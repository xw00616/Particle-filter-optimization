...
% function [Output,Boundary,Coding] = P_OneMax(Operation,Problem,M,Input)
% 
% Boundary = NaN; Coding = NaN;
% switch Operation
%     %Population Initialization
...
%         PopDec   = Input;
%         PopObj = zeros(size(PopDec,1),M);
%         R = round(rand(size(PopDec,1),size(PopDec,2)));
%         switch Problem
%             case 'OneMax1' %%corr=-0.5   
%                 PopObj(:,1) = sum(PopDec,2);
%                 b = abs(PopDec);
%                 PopObj(:,2) = sum(b,2);
%             case 'OneMax2'%%corr=0.5 
%                PopObj(:,1) = sum(PopDec,2);
%                 b = abs(PopDec-ones(size(PopDec,1),size(PopDec,2)));
%                 PopObj(:,2) = sum(b,2);
%             case 'OneMax3'%%corr=0
%                  PopObj(:,1) = sum(PopDec,2);
%                 b = abs(PopDec-ones(size(PopDec,1),size(PopDec,2)));
